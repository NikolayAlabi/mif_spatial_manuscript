...
def standardize_and_validate_coord_df(
    ...
    *,
    ...
    # Standardize all coords, including BLASST underscore form
    out[coord_col] = standardize_coord_series(out[coord_col])
...
DEFAULT_COHORTS = ["PURE01", "NAC2015", "NAC2020", "No-NAC", "BLASST"]
DEFAULT_PANELS = ["AR", "BT"]

DEFAULT_PHENOTYPE_SPATIAL_ROOT = Path("/projects/ovcare/users/nikolay_alabi/immuno/weibull/run_reviewed_phenotype_only")
DEFAULT_AR_STATE_SPATIAL_ROOT = Path("/projects/ovcare/users/nikolay_alabi/immuno/weibull/run_reviewed_AR_state")

DEFAULT_PHENOTYPE_CELL_FEATURES_PATH = Path(
    "/projects/ovcare/users/nikolay_alabi/immuno/cell_feature_tables_reviewed/phenotype_only_from_prep_allneg_ratios/cell_features_phenotype_only_from_prep_allneg_ratios.csv"
)
DEFAULT_AR_STATE_CELL_FEATURES_PATH = Path(
    "/projects/ovcare/users/nikolay_alabi/immuno/cell_feature_tables_reviewed/AR_state_from_prep_allneg_ratios/cell_features_AR_state_from_prep_allneg_ratios.csv"
)
DEFAULT_PHENOTYPE_TRIADS_PATH = Path(
    "/projects/ovcare/users/nikolay_alabi/immuno/cell_feature_tables_reviewed/triads_phenotype_only_all_labels/triad_features_phenotype_only_all_labels.csv"
)
DEFAULT_AR_STATE_TRIADS_PATH = Path(
    "/projects/ovcare/users/nikolay_alabi/immuno/cell_feature_tables_reviewed/triads_AR_state_all_labels/triad_features_AR_state_all_labels.csv"
)
...
def _default_spatial_root_for_source(feature_source: str) -> Path:
    if feature_source == "phenotype_only":
        return DEFAULT_PHENOTYPE_SPATIAL_ROOT
    if feature_source == "AR_state":
        return DEFAULT_AR_STATE_SPATIAL_ROOT
...
def _default_cell_features_path_for_source(feature_source: str) -> Path:
    if feature_source == "phenotype_only":
        return DEFAULT_PHENOTYPE_CELL_FEATURES_PATH
    if feature_source == "AR_state":
        return DEFAULT_AR_STATE_CELL_FEATURES_PATH
    raise ValueError(f"Unknown feature_source: {feature_source}")


def _default_triads_path_for_source(feature_source: str) -> Path:
    if feature_source == "phenotype_only":
        return DEFAULT_PHENOTYPE_TRIADS_PATH
    if feature_source == "AR_state":
        return DEFAULT_AR_STATE_TRIADS_PATH
    raise ValueError(f"Unknown feature_source: {feature_source}")
...
def prepare_core_level_feature_table(
    ...
    # Use cohort-appropriate metadata sources
    if str(cohort) == "BLASST":
    ...
    feat_df = standardize_and_validate_coord_df(feat_df, "feature_df")
    ...
    # Filter feature table to requested cohort / panel
    if "cohort" in feat_df.columns:
        feat_df = feat_df[feat_df["cohort"].astype(str) == str(cohort)].copy()
    if "Panel" in feat_df.columns:
        feat_df = feat_df[feat_df["Panel"].astype(str) == str(panel)].copy()
...
def replace_with_harmonized_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols_to_drop = set(harm_cols) | set(base_names)
...
def run_job(cohort: str, 
    ...
    core_df = prepare_core_level_feature_table(data_dict=data_dict, feature_group=feature_group, cohort=cohort, panel=panel, qc_acceptability=qc_acceptability, min_epi_fraction=min_epi_fraction,sample_type=sample_type)
    if core_df.empty:
    ...
    if cohort == "No-NAC" and endpoint in {"OS", "RFS"}:
    ...
    if endpoint in {"complete_response", "any_response"}:
        ...
        for i, feature_name in enumerate(feature_cols, start=1):
            ...
            if res["summary"] is not None:
                ...
                row.update({"cohort": cohort, "panel": panel, "feature_source": feature_source, "feature_group": feature_group, "endpoint": endpoint, "patient_subset": patient_subset, "sample_type": sample_type, "qc_acceptability": qc_acceptability, "min_epi_fraction": min_epi_fraction, "agg": agg, "n_splits": n_splits, "n_repeats": n_repeats, "chunk_idx": chunk_idx, "n_chunks": n_chunks})
                summary_rows.append(row)
            if isinstance(res["fold_df"], pd.DataFrame) and not res["fold_df"].empty:
                tmp = res["fold_df"].copy(); tmp["cohort"] = cohort; tmp["panel"] = panel; tmp["feature_source"] = feature_source; tmp["feature_group"] = feature_group; tmp["endpoint"] = endpoint; tmp["patient_subset"] = patient_subset; tmp["sample_type"] = sample_type;  tmp["chunk_idx"] = chunk_idx; fold_parts.append(tmp)
            if isinstance(res["oof_df"], pd.DataFrame) and not res["oof_df"].empty:
                tmp = res["oof_df"].copy(); tmp["cohort"] = cohort; tmp["panel"] = panel; tmp["feature_source"] = feature_source; tmp["feature_group"] = feature_group; tmp["endpoint"] = endpoint; tmp["patient_subset"] = patient_subset; tmp["sample_type"] = sample_type; tmp["chunk_idx"] = chunk_idx; oof_parts.append(tmp)
            if res["status"] in {"skip", "fail"}:
                fail_rows.append({"feature": feature_name, "status": res["status"], "reason": res.get("reason", ""), "cohort": cohort, "panel": panel, "feature_source": feature_source, "feature_group": feature_group, "endpoint": endpoint, "patient_subset": patient_subset, "sample_type": sample_type, "chunk_idx": chunk_idx})
    ...
    stem = f"{cohort}__{panel}__{feature_source}__{feature_group}__{endpoint}__{sample_type}__{patient_subset}__agg-{agg}__chunk{chunk_idx:03d}of{n_chunks:03d}"
    summary_df = pd.DataFrame(summary_rows)
    fold_df = pd.concat(fold_parts, ignore_index=True) if fold_parts else pd.DataFrame()
    oof_df = pd.concat(oof_parts, ignore_index=True) if oof_parts else pd.DataFrame()
    ...
    runmeta = {
        ...
        "qc_acceptability": qc_acceptability, "min_epi_fraction": min_epi_fraction, "agg": agg,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        choices=["phenotype_only", "AR_state"],
