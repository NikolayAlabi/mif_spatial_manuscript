...
# -----------------------------------------------------------------------------
# V2 table builders: clinical-only Table 1 and comprehensive mIF Table 2
# -----------------------------------------------------------------------------

CLINICAL_TABLE1_ROW_DEFS = [
    ("Treatment context", "treatment_context"),
    ("Clinical patients, n", "clinical_patients_n"),
    ("Age, median (IQR), years", "age_median_iqr"),
    ("Male sex, n (%)", "male_n_pct"),
    ...
    ("OS evaluable, n", "OS_evaluable_n"),
    ("OS events, n (%)", "OS_events_n_pct"),
    ("OS time/follow-up, median (IQR), months", "OS_time_median_iqr_months"),
    ("KM median OS, months (if reached)", "KM_median_OS_months"),
    ("RFS evaluable, n", "RFS_evaluable_n"),
    ("RFS events, n (%)", "RFS_events_n_pct"),
    ("RFS time/follow-up, median (IQR), months", "RFS_time_median_iqr_months"),
    ("KM median RFS, months (if reached)", "KM_median_RFS_months"),
    ("Adjuvant chemotherapy, n (%)", "adjuvant_chemo_n_pct"),
    ("Adjuvant IO, n (%)", "adjuvant_IO_n_pct"),
]


def format_value_for_wide(val) -> str:
    """Format scalar values for manuscript/report tables."""
    if isinstance(val, pd.Series):
        val = val.iloc[0]
...
def make_wide_from_row_defs(compact: pd.DataFrame, cohorts: list[str], row_defs: list[tuple[str, str]]) -> pd.DataFrame:
    idx = compact.set_index("cohort")
    rows = []
    for label, col in row_defs:
        row = {"Characteristic": label}
        for cohort in cohorts:
            if cohort in idx.index and col in compact.columns:
                row[cohort] = format_value_for_wide(idx.loc[cohort, col])
            else:
                row[cohort] = "NA"
        rows.append(row)
    return pd.DataFrame(rows)
...
def make_clinical_table1_compact(clinical_summary: pd.DataFrame, cohorts: list[str]) -> pd.DataFrame:
    """Return one-row-per-cohort clinical-only Table 1 compact source table."""
    keep = ["cohort"] + [col for _, col in CLINICAL_TABLE1_ROW_DEFS if col in clinical_summary.columns]
    out = clinical_summary[keep].copy()
    out["cohort"] = pd.Categorical(out["cohort"], categories=cohorts, ordered=True)
    return out.sort_values("cohort").reset_index(drop=True)


def make_clinical_table1_wide(clinical_compact: pd.DataFrame, cohorts: list[str]) -> pd.DataFrame:
    return make_wide_from_row_defs(clinical_compact, cohorts, CLINICAL_TABLE1_ROW_DEFS)


def add_table2_display_columns(mif_cohort_summary: pd.DataFrame) -> pd.DataFrame:
    """Add display strings and derived columns to the cohort-level mIF summary."""
    out = mif_cohort_summary.copy()

    for st in SAMPLE_TYPES:
        for stage in ["submitted", "analysis_ready"]:
            mean_col = f"{st}_cores_sections_per_patient_{stage}_mean"
            display_col = f"{st}_cores_sections_per_patient_{stage}_mean_display"
            out[display_col] = out[mean_col].map(lambda x: fmt_float(x, 1)) if mean_col in out.columns else "NA"

            med_col = f"{st}_cores_sections_per_patient_{stage}_median_iqr"
            if med_col not in out.columns:
                out[med_col] = "NA"

    for panel in REPORT_PANELS:
        sub_col = f"{panel}_panel_units_submitted_n"
        ready_col = f"{panel}_panel_units_analysis_ready_n"
        pct_col = f"{panel}_panel_units_analysis_ready_n_pct"
        if sub_col in out.columns and ready_col in out.columns:
            out[pct_col] = out.apply(lambda r: clean_percent(r[ready_col], r[sub_col]), axis=1)

    return out


def make_table2_mif_compact(
    mif_cohort_summary: pd.DataFrame,
    mif_panel_summary: pd.DataFrame,
    cohorts: list[str],
    panels: list[str],
) -> pd.DataFrame:
    """
    Comprehensive cohort-level mIF table.

    This intentionally mixes:
      - cohort-level mIF patient/physical unit counts
      - sample-type-specific TURBT/RC counts
      - panel/sample-type panel-unit QC counts
    """
    base = add_table2_display_columns(mif_cohort_summary)

    # Add panel/sample-type details as wide columns.
    detail = mif_panel_summary.copy()
    if not detail.empty:
        for c in ["cohort", "sample_type", "panel"]:
            detail[c] = detail[c].astype(str)

        wide_rows = []
        for cohort in cohorts:
            row = {"cohort": cohort}
            subc = detail[detail["cohort"].eq(cohort)]
            for panel in panels:
                for st in SAMPLE_TYPES:
                    sub = subc[(subc["panel"].eq(panel)) & (subc["sample_type"].eq(st))]
                    prefix = f"{panel}_{st}"
                    if sub.empty:
                        row[f"{prefix}_panel_units_submitted_n"] = 0
                        row[f"{prefix}_panel_units_analysis_ready_n_pct"] = "0"
                        row[f"{prefix}_patients_submitted_n"] = 0
                        row[f"{prefix}_segmented_cells_n"] = 0
                        row[f"{prefix}_acceptable_n"] = 0
                        row[f"{prefix}_borderline_n"] = 0
                        row[f"{prefix}_unusable_n"] = 0
                        row[f"{prefix}_missing_qc_n"] = 0
                    else:
                        r = sub.iloc[0]
                        submitted = int(r.get("panel_units_submitted_n", 0))
                        ready = int(r.get("panel_units_analysis_ready_n", 0))
                        row[f"{prefix}_panel_units_submitted_n"] = submitted
                        row[f"{prefix}_panel_units_analysis_ready_n_pct"] = clean_percent(ready, submitted)
                        row[f"{prefix}_patients_submitted_n"] = int(r.get("patients_submitted_n", 0))
                        row[f"{prefix}_segmented_cells_n"] = int(r.get("segmented_cells_n", 0))
                        row[f"{prefix}_acceptable_n"] = int(r.get("n_acceptable", 0))
                        row[f"{prefix}_borderline_n"] = int(r.get("n_borderline", 0))
                        row[f"{prefix}_unusable_n"] = int(r.get("n_unusable", 0))
                        row[f"{prefix}_missing_qc_n"] = int(r.get("n_missing_qc", 0))
            wide_rows.append(row)
        panel_wide = pd.DataFrame(wide_rows)
        base = base.merge(panel_wide, on="cohort", how="left")

    base["cohort"] = pd.Categorical(base["cohort"], categories=cohorts, ordered=True)
    return base.sort_values("cohort").reset_index(drop=True)


def make_table2_mif_wide(table2_compact: pd.DataFrame, cohorts: list[str], panels: list[str]) -> pd.DataFrame:
    row_defs = [
        ("Specimen format", "specimen_format"),
        ("Patients with mIF cores/sections submitted, n", "mif_patients_submitted_n"),
        ("Patients analysis-ready after QC/source reconciliation, n", "mif_patients_analysis_ready_n"),
        ("Patients lost at mIF QC/source reconciliation, n (%)", "mif_patients_lost_at_qc_n_pct"),
        ("Physical TMA cores/whole-section regions submitted, n", "physical_cores_sections_submitted_n"),
        ("Physical TMA cores/whole-section regions analysis-ready, n (%)", "physical_cores_sections_analysis_ready_pct"),
        ("TURBT patients submitted, n", "TURBT_patients_submitted_n"),
        ...
        ("TURBT cores/sections per patient submitted, mean", "TURBT_cores_sections_per_patient_submitted_mean_display"),
        ("TURBT cores/sections per patient analysis-ready, mean", "TURBT_cores_sections_per_patient_analysis_ready_mean_display"),
        ("TURBT cores/sections per patient submitted, median (IQR)", "TURBT_cores_sections_per_patient_submitted_median_iqr"),
        ("TURBT cores/sections per patient analysis-ready, median (IQR)", "TURBT_cores_sections_per_patient_analysis_ready_median_iqr"),
        ("RC patients submitted, n", "RC_patients_submitted_n"),
        ("RC patients analysis-ready, n", "RC_patients_analysis_ready_n"),
        ("RC cores/sections submitted, n", "RC_cores_sections_submitted_n"),
        ("RC cores/sections analysis-ready, n", "RC_cores_sections_analysis_ready_n"),
        ("RC cores/sections per patient submitted, mean", "RC_cores_sections_per_patient_submitted_mean_display"),
        ("RC cores/sections per patient analysis-ready, mean", "RC_cores_sections_per_patient_analysis_ready_mean_display"),
        ("RC cores/sections per patient submitted, median (IQR)", "RC_cores_sections_per_patient_submitted_median_iqr"),
        ("RC cores/sections per patient analysis-ready, median (IQR)", "RC_cores_sections_per_patient_analysis_ready_median_iqr"),
        ("Matched TURBT-RC patients submitted, n", "matched_TURBT_RC_patients_submitted_n"),
        ("Matched TURBT-RC patients analysis-ready, n", "matched_TURBT_RC_patients_analysis_ready_n"),
    ]
    ...
    for panel in panels:
        row_defs += [
            (f"{panel} panel units submitted, n", f"{panel}_panel_units_submitted_n"),
            (f"{panel} panel units analysis-ready, n (%)", f"{panel}_panel_units_analysis_ready_n_pct"),
            (f"{panel} segmented cells, n", f"{panel}_segmented_cells_n"),
        ]
        for st in SAMPLE_TYPES:
            prefix = f"{panel}_{st}"
            row_defs += [
                (f"{panel} {st} panel units submitted, n", f"{prefix}_panel_units_submitted_n"),
                (f"{panel} {st} panel units analysis-ready, n (%)", f"{prefix}_panel_units_analysis_ready_n_pct"),
                (f"{panel} {st} patients submitted, n", f"{prefix}_patients_submitted_n"),
                (f"{panel} {st} segmented cells, n", f"{prefix}_segmented_cells_n"),
                (f"{panel} {st} acceptable cores/sections, n", f"{prefix}_acceptable_n"),
                (f"{panel} {st} borderline cores/sections, n", f"{prefix}_borderline_n"),
                (f"{panel} {st} unusable cores/sections, n", f"{prefix}_unusable_n"),
                (f"{panel} {st} missing structural QC, n", f"{prefix}_missing_qc_n"),
            ]
    ...
    return make_wide_from_row_defs(table2_compact, cohorts, row_defs)


def make_survival_qc_note_table(clinical_summary: pd.DataFrame, cohorts: list[str]) -> pd.DataFrame:
    """
    Make an explicit note table showing why KM medians may be NR.

    A KM median is NR when the survival curve does not cross 50%, which often
    happens when fewer than half the cohort experiences the event.
    """
    rows = []
    for _, r in clinical_summary.iterrows():
        cohort = r["cohort"]
        if cohort not in cohorts:
            continue
        for endpoint in ["OS", "RFS"]:
            eval_col = f"{endpoint}_evaluable_n"
            event_col = f"{endpoint}_events_n_pct"
            km_col = f"KM_median_{endpoint}_months"
            if eval_col not in clinical_summary.columns:
                continue
            rows.append({
                "cohort": cohort,
                "endpoint": endpoint,
                "evaluable_n": r.get(eval_col, np.nan),
                "events_n_pct": r.get(event_col, "NA"),
                "km_median_months": r.get(km_col, "NA"),
                "interpretation": (
                    "median reached" if str(r.get(km_col, "")).upper() not in {"NR", "NA", ""} 
                    else "KM median not reached; survival curve did not cross 50%"
                ),
            })
    return pd.DataFrame(rows)
...
def build_tables(
    ...
    # Table 1 is now clinical-only.
    table1_compact = make_clinical_table1_compact(clinical_summary, cohorts)
    table1_wide = make_clinical_table1_wide(table1_compact, cohorts)

    # Table 2 is the comprehensive mIF availability/QC table.
    table2_compact = make_table2_mif_compact(
        mif_cohort_summary=mif_cohort_summary,
        mif_panel_summary=mif_panel_summary,
        cohorts=cohorts,
        panels=panels,
    )
    table2_wide = make_table2_mif_wide(table2_compact, cohorts, panels)

    survival_note = make_survival_qc_note_table(clinical_summary, cohorts)
    dictionary = write_dictionary(out_dir)

    # Save Table 1 clinical-only. For backwards compatibility, also overwrite
    # the older expanded table names with the clinical-only versions.
    table1_wide.to_csv(out_dir / "table1_clinical_by_cohort_wide.csv", index=False)
    table1_compact.to_csv(out_dir / "table1_clinical_by_cohort_compact.csv", index=False)
    table1_wide.to_csv(out_dir / "table1_report_by_cohort_expanded_wide.csv", index=False)
    table1_compact.to_csv(out_dir / "table1_report_by_cohort_expanded_compact.csv", index=False)

    # Save Table 2 mIF.
    table2_wide.to_csv(out_dir / "table2_mif_qc_availability_by_cohort_wide.csv", index=False)
    table2_compact.to_csv(out_dir / "table2_mif_qc_availability_by_cohort_compact.csv", index=False)

    # Preserve and expand detailed/source outputs.
    ...
    mif_panel_summary.to_csv(out_dir / "table2_mif_panel_unit_detail.csv", index=False)
    clinical_summary.to_csv(out_dir / "source_data_table1_clinical_expanded.csv", index=False)
    mif_units.to_csv(out_dir / "source_data_table2_mif_patient_unit_level.csv", index=False)
    mif_units.to_csv(out_dir / "source_data_table1_mif_patient_unit_level.csv", index=False)
    survival_note.to_csv(out_dir / "table1_survival_km_median_notes.csv", index=False)

    # Optional warning table for ID fallback.
    warnings = []
    if "patient_ids_requiring_fallback_n" in table2_compact.columns:
        for _, r in table2_compact.iterrows():
            n = r.get("patient_ids_requiring_fallback_n", 0)
            if pd.notna(n) and int(n) > 0:
                warnings.append({
                    "cohort": r["cohort"],
                    "warning": f"{int(n)} mIF patient IDs required fallback inference. Review source_data_table2_mif_patient_unit_level.csv before final reporting.",
                })
    warn_df = pd.DataFrame(warnings)
    warn_df.to_csv(out_dir / "table2_patient_id_mapping_warnings.csv", index=False)
...
def main():
    ...
    print("\nWrote clinical Table 1 and mIF Table 2 outputs to:", Path(args.out_dir))
    print("\nPreview:")
    print("\nTable 1 clinical-only:")
    print(outputs["table1_wide"].to_string(index=False))
    print("\nTable 2 mIF availability/QC:")
    print(outputs["table2_wide"].to_string(index=False))
