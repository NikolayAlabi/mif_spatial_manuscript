#!/bin/bash
# Submit corrected Stage 2A-4 v2 workers only.
# No dependency job is created. Run aggregate manually after all workers finish.

set -euo pipefail

MODULE_DIR="${MODULE_DIR:-/projects/ovcare/users/nikolay_alabi/immuno/manuscript/modules}"
CONFIG_JSON="${CONFIG_JSON:-$MODULE_DIR/configs/stage2a4_filter_build_context_matrices_v2.json}"
SCRIPT="${SCRIPT:-$MODULE_DIR/stage2a4_filter_build_context_matrices_v2.py}"
CONTEXT_INDEX="${CONTEXT_INDEX:-/projects/ovcare/users/nikolay_alabi/immuno/stage2_global_modules_v8/stage2a_steps1_3_context_review_v4/stage2a_context_index.csv}"
LOGDIR="${LOGDIR:-/projects/ovcare/users/nikolay_alabi/immuno/stage2_global_modules_v9/logs/stage2a4_v2}"
...
[[ -s "$CONTEXT_INDEX" ]] || { echo "[ERROR] missing context index: $CONTEXT_INDEX" >&2; exit 1; }

N_CONTEXTS=$(awk 'END {print NR - 1}' "$CONTEXT_INDEX")
[[ "$N_CONTEXTS" -gt 0 ]] || { echo "[ERROR] no contexts found" >&2; exit 1; }
LAST=$((N_CONTEXTS - 1))

SLURM_SCRIPT="$LOGDIR/stage2a4_v2_workers.slurm.sh"

cat > "$SLURM_SCRIPT" <<EOF
#!/bin/bash
#SBATCH --job-name=s2a4v2
