#!/usr/bin/env python3
"""
stage2a5_interpretable_microcompression_v2.py
...
def exact_duplicate_pass(
    ...
    for uid in active:
        ...
        semantic_key = str(
            meta.loc[uid].get(
                "exact_semantic_key",
                meta.loc[uid].get("full_semantic_key", ""),
            )
        )
        groups.setdefault(
            (semantic_key, vector_hash(matrix[uid])),
            [],
        ).append(uid)

    atol = float(cfg.get("exact_duplicate_atol", 1e-12))
    for (semantic_key, hash_value), uids in groups.items():
        if len(uids) < 2:
            continue
        ...
        for uid in uids:
            ...
            audit.append({
                ...
                "decision_reason": "same corrected semantic identity and identical transformed patient vector; simplest/highest-evidence representation retained",
            })
...
def command_worker(cfg: Mapping, array_id: int) -> None:
    ...
    if not parser_problem.empty:
        parser_problem.to_csv(
            context_dir / "ERROR_microcompression_parser_failures.csv",
            index=False,
        )
        raise RuntimeError(
            "{} registry feature(s) failed corrected parser in {}".format(
                len(parser_problem), row["context_id"]
            )
        )
