#!/usr/bin/env python3
"""
stage2a4_filter_build_context_matrices_v2.py
...
Examples
--------
python stage2a4_filter_build_context_matrices_v2.py init-rules --config CONFIG.json
python stage2a4_filter_build_context_matrices_v2.py validate --config CONFIG.json
python stage2a4_filter_build_context_matrices_v2.py worker --config CONFIG.json --array-id 0
python stage2a4_filter_build_context_matrices_v2.py aggregate --config CONFIG.json
"""
...
def get_interpretability_utils(cfg: Mapping):
    ...
    if path is None:
        path = Path(__file__).with_name("stage2a_interpretability_utils_v2.py")
...
def command_validate(cfg: Mapping) -> None:
    ...
    iu_path = Path(
        cfg.get(
            ...
            Path(__file__).with_name("stage2a_interpretability_utils_v2.py"),
        )
    )
    parser_path = Path(
        cfg.get(
            "feature_parser_path",
            Path(__file__).with_name("stage2_feature_parser_v8.py"),
        )
    )
