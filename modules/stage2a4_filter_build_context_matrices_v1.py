#!/usr/bin/env python3
"""
stage2a4_filter_build_context_matrices_v1.py
...
Examples
--------
python stage2a4_filter_build_context_matrices_v1.py init-rules --config CONFIG.json
python stage2a4_filter_build_context_matrices_v1.py validate --config CONFIG.json
python stage2a4_filter_build_context_matrices_v1.py worker --config CONFIG.json --array-id 0
python stage2a4_filter_build_context_matrices_v1.py aggregate --config CONFIG.json
"""
...
def command_worker(cfg: Mapping, array_id: int) -> None:
    ...
    seeds, filter_audit = apply_context_thresholds(features, rule)
    iu = get_interpretability_utils(cfg)
    rescues, rescue_links = identify_rescue_alternatives(features, seeds, rule, cfg, iu)
    ...
    if bool(cfg.get("skip_matrix_build", False)):
        ...
        matrix, matrix_meta = build_patient_matrix(registry, cfg, context_dir)
        if matrix.empty or matrix.shape[1] <= 1:
            status = "matrix_build_zero_features"
        else:
            status = "complete"
            save_table(matrix, context_dir / "patient_feature_matrix.parquet")
        matrix_meta.to_csv(context_dir / "matrix_feature_meta.csv", index=False)
