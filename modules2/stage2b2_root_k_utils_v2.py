...
def summarize_k_recommendation(kdiag: pd.DataFrame) -> dict:
    ...
    best_row = d.sort_values(["k_composite_score", "requested_k"], ascending=[False, True]).iloc[0]
    near = d[d["within_best_fraction"]].copy()
    near_healthy = d[d["within_best_and_size_healthy"]].copy()
    if not near_healthy.empty:
        pars = near_healthy.sort_values("requested_k").iloc[0]
        pars_reason = "smallest_k_within_score_fraction_and_size_healthy"
    elif not near.empty:
        pars = near.sort_values("requested_k").iloc[0]
        pars_reason = "smallest_k_within_score_fraction_no_size_healthy_candidate"
    else:
        pars = best_row
        pars_reason = "best_score_only"

    return {
        "panel": best_row["panel"],
        "feature_source": best_row["feature_source"],
        "distance_mode": best_row["distance_mode"],
        "n_features": int(best_row["n_features"]),
        "best_score_k": int(best_row["requested_k"]),
        "best_score": float(best_row["k_composite_score"]),
        "recommended_parsimonious_k": int(pars["requested_k"]),
        "recommendation_reason": pars_reason,
        "recommended_score": float(pars["k_composite_score"]),
...
def plot_k_diagnostics(
    ...
    *,
    ...
    fig, ax = plt.subplots(figsize=(8.5, 5.2))
    ax.plot(d["requested_k"], d["k_composite_score"], marker="o")
    best = d.loc[d["k_composite_score"].idxmax()]
    rec = summarize_k_recommendation(d)
    ax.axvline(int(best["requested_k"]), linestyle="--", linewidth=1, label=f"Best score K={int(best['requested_k'])}")
    ax.axvline(int(rec["recommended_parsimonious_k"]), linestyle=":", linewidth=1.5, label=f"Parsimonious K={int(rec['recommended_parsimonious_k'])}")
    ax.set_xlabel("K")
    ax.set_ylabel("Rank-normalized composite K score")
    ax.set_title(f"{prefix}: overall K score\n(45% silhouette; 30% primitive; 10% tissue; 15% size health)")
    ax.legend(frameon=False)
    fig.tight_layout()
...
def run_full_stage2b2_review(
    *,
    ...
    settings = {
        ...
        "semantic_composite_policy": "45% statistical silhouette + 30% primitive coherence enrichment + 10% tissue-aware coherence enrichment + 7.5% largest-cluster control + 7.5% singleton control; metric-aware coherence and all semantic silhouettes diagnostic only",
