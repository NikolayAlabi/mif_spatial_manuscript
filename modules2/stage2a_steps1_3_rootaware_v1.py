#!/usr/bin/env python3
"""
stage2a_steps1_3_rootaware_v1.py

Stage 2A, steps 1-3 ROOT-AWARE review with bundle-aware context outputs:
  ...
  3. Compute both global-context and within-prep-root evidence rankings for MANUAL review.

This script deliberately stops before root-specific nomination, within-root microcompression,
and root-module discovery. It does not decide which contexts enter global module
...
1) Build inventory and context shards:
   python stage2a_steps1_3_rootaware_v1.py inventory --config CONFIG.json

2) Run one context (or use SLURM_ARRAY_TASK_ID):
   python stage2a_steps1_3_rootaware_v1.py worker --config CONFIG.json --array-id 0

3) Aggregate completed context outputs:
   python stage2a_steps1_3_rootaware_v1.py aggregate --config CONFIG.json
"""
...
# Root-specific biological spaces used by the finalized root-module -> meta-module design.
# These are deliberately treated as separate measurement ontologies downstream.
EXPECTED_PREP_ROOTS_BY_PANEL = {
    "AR": [
        "phenotype_only",
        "AR_state",
        "AR_checkpoint_state",
...
DEFAULT_TECHNICAL_FEATURE_PATTERNS = [
    r"^(All|Epi|Stroma)__n_cells$",
    r"^(All|Epi|Stroma)__n_resolved_for_ratio$",
    r"^n_cells_total_input$",
]
...
def _add_evidence_columns(
    ...
    """
    Add percentile-based evidence components to one comparison universe.

    prefix="" gives the legacy/global context-level columns.
    prefix="root_" gives within-prep-root columns.
    """
    out = out.copy()

    out[f"{prefix}rank_oof"] = percentile_rank(
        out["oof_metric"], higher_better=True
    )
    out[f"{prefix}rank_delta_clinical"] = percentile_rank(
        out["delta_clinical"], higher_better=True
    )
    out[f"{prefix}rank_fold_sd"] = percentile_rank(
        out["fold_sd"], higher_better=False
    )
    out[f"{prefix}rank_direction_consistency"] = percentile_rank(
        out["direction_consistency"], higher_better=True
    )
    out[f"{prefix}rank_completeness"] = percentile_rank(
        out["nonmissing_fraction"], higher_better=True
    )

    stability_parts = pd.concat(
        [
            out[f"{prefix}rank_fold_sd"],
            out[f"{prefix}rank_direction_consistency"],
        ],
        axis=1,
    )
    out[f"{prefix}rank_stability"] = stability_parts.mean(
        axis=1, skipna=True
    )

    weights = cfg.get("score_weights", {})
    components = {
        f"{prefix}rank_oof": float(weights.get("oof", 0.55)),
        f"{prefix}rank_stability": float(weights.get("stability", 0.20)),
        f"{prefix}rank_delta_clinical": float(weights.get("delta_clinical", 0.15)),
        f"{prefix}rank_completeness": float(weights.get("completeness", 0.10)),
    }
    ...
    score_col = (
        ...
        if prefix == ""
        else f"{prefix}candidate_evidence_score"
    )
    weight_col = (
        "evidence_score_weight_available"
        if prefix == ""
        else f"{prefix}evidence_score_weight_available"
    )

    out[score_col] = numerator / denominator.replace(0, np.nan)
    out[weight_col] = denominator
    return out


def add_candidate_evidence_score(
    selected: pd.DataFrame,
    cfg: Mapping,
) -> pd.DataFrame:
    """
    Add both:
      1. global-context evidence ranks/scores (legacy audit view), and
      2. within-prep-root evidence ranks/scores (PRIMARY nomination view).

    Prep roots therefore no longer compete for the same percentile ranks.
    """
    out = selected.copy()
    if out.empty:
        return out

    # Global/context ranking retained for auditability and comparison to v4.
    out = _add_evidence_columns(out, cfg, prefix="")
    ...
    # Root-specific rankings: PRIMARY ranking for future root nomination.
    root_parts = []
    for feature_source, g in out.groupby(
        "feature_source",
        dropna=False,
        sort=False,
    ):
        gg = _add_evidence_columns(g, cfg, prefix="root_")
        gg = gg.sort_values(
            [
        ...
        gg["root_candidate_rank"] = np.arange(1, len(gg) + 1)
        root_parts.append(gg)

    out = pd.concat(root_parts, ignore_index=True, sort=False)

    # Restore deterministic global ordering for saved tables.
    out = out.sort_values(
        [
    ...
    stable_direction_cutoff = float(
        cfg.get("stable_direction_cutoff", 0.80)
    )
    stable_fold_sd_cutoff = float(
        cfg.get("stable_fold_sd_cutoff", 0.10)
    )

    out["stable_direction_flag"] = (
        out["direction_consistency"] >= stable_direction_cutoff
    )
    out["low_fold_sd_flag"] = (
        out["fold_sd"] <= stable_fold_sd_cutoff
    )
    out["stable_flag"] = (
        out["stable_direction_flag"].fillna(False)
        | out["low_fold_sd_flag"].fillna(False)
    )
...
def build_root_candidate_summary(
    selected: pd.DataFrame,
    array_id: int,
    cfg: Mapping,
) -> pd.DataFrame:
    """One row per context x prep root after best-transform selection."""
    if selected.empty:
        return pd.DataFrame()

    rows = []
    for feature_source, g in selected.groupby(
        "feature_source",
        dropna=False,
        sort=True,
    ):
        ...
        root_score = safe_numeric(g["root_candidate_evidence_score"])
        ...
        row = {
            ...
            **{col: first.get(col) for col in CONTEXT_COLS},
            ...
            "n_best_transform_variables": int(g["feature_uid"].nunique()),
            "n_feature_groups": int(g["feature_group"].nunique()),
            "max_oof": float(oof.max()) if oof.notna().any() else np.nan,
            "median_oof": float(oof.median()) if oof.notna().any() else np.nan,
            "median_top5_oof": top_median(ordered["oof_metric"], 5),
            "median_top10_oof": top_median(ordered["oof_metric"], 10),
            "median_top20_oof": top_median(ordered["oof_metric"], 20),
            "best_root_candidate_evidence_score": (
                float(root_score.max()) if root_score.notna().any() else np.nan
            ),
            "median_top10_root_candidate_evidence_score": top_median(
                ordered["root_candidate_evidence_score"], 10
            ),
            "n_stable": int(g["stable_flag"].fillna(False).sum()),
            "n_nominal_p_le_005": int((g["p_value"] <= 0.05).sum()),
            "n_context_q_le_020": int((g["context_q_value"] <= 0.20).sum()),
            "n_zscore_selected": int(
                (g["selected_transform_mode"] == "zscore").sum()
            ),
            "n_log1p_zscore_selected": int(
                (g["selected_transform_mode"] == "log1p_zscore").sum()
            ),
        }

        for threshold in cfg.get(
            "oof_review_thresholds",
            [0.55, 0.60, 0.65],
        ):
            label = "{:03d}".format(
                int(round(float(threshold) * 100))
            )
            row[f"n_oof_ge_{label}"] = int(
                (oof >= float(threshold)).sum()
            )
...
def build_root_candidate_depth_profile(
    selected: pd.DataFrame,
    array_id: int,
    cfg: Mapping,
) -> pd.DataFrame:
    """
    Describe the quality drop-off as progressively deeper candidates are
    considered within each prep root. Used to choose later root-specific caps.
    """
    if selected.empty:
        return pd.DataFrame()
    ...
    for feature_source, g in selected.groupby(
        ...
        for requested_depth in sorted(
            set(int(x) for x in depths if int(x) > 0)
        ):
            sub = ordered.head(requested_depth)
            if sub.empty:
                continue
            ...
            rows.append({
                ...
                **{col: first.get(col) for col in CONTEXT_COLS},
                ...
                "n_available_at_depth": int(len(sub)),
                "root_is_exhausted": bool(len(ordered) <= requested_depth),
                "nth_root_candidate_evidence_score": last.get(
                    "root_candidate_evidence_score"
                ),
                "nth_oof_metric": last.get("oof_metric"),
                "median_oof_topn": safe_numeric(
                    sub["oof_metric"]
                ).median(),
                "min_oof_topn": safe_numeric(
                    sub["oof_metric"]
                ).min(),
                "median_root_evidence_topn": safe_numeric(
                    sub["root_candidate_evidence_score"]
                ).median(),
                "median_fold_sd_topn": safe_numeric(
                    sub["fold_sd"]
                ).median(),
                "median_nonmissing_topn": safe_numeric(
                    sub["nonmissing_fraction"]
                ).median(),
                "n_stable_topn": int(
                    sub["stable_flag"].fillna(False).sum()
                ),
            })
...
def remove_excluded_technical_features(
    df: pd.DataFrame,
    cfg: Mapping,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Remove technical/QC bookkeeping variables before transform selection
    and evidence ranking.

    Defaults explicitly cover the technical fields discovered during the v9
    parser audit. Additional exact names/patterns can be supplied in config.
    """
    patterns = list(DEFAULT_TECHNICAL_FEATURE_PATTERNS)
    patterns.extend(
        str(x) for x in cfg.get(
            "excluded_technical_feature_patterns",
            [],
        )
    )
    ...
    exact_names = {
        str(x).strip()
        for x in cfg.get(
            "excluded_technical_feature_names",
            [],
        )
    }

    feature = df["feature"].astype(str).str.strip()

    exact_mask = feature.isin(exact_names)
    pattern_mask = pd.Series(False, index=df.index)

    for pattern in patterns:
        pattern_mask |= feature.str.match(
            pattern,
            case=False,
            na=False,
        )
    ...
    excluded_mask = exact_mask | pattern_mask
    return (
        df.loc[~excluded_mask].copy(),
        df.loc[excluded_mask].copy(),
    )


def validate_panel_prep_roots(
    df: pd.DataFrame,
    cfg: Mapping,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Enforce the finalized panel-specific prep-root universe.

    AR:
      phenotype_only, AR_state, AR_checkpoint_state,
      compartment, compartment_state

    BT:
      phenotype_only, compartment
    """
    allowed_map = cfg.get(
        "expected_prep_roots_by_panel",
        EXPECTED_PREP_ROOTS_BY_PANEL,
    ...
    for panel, idxs in df.groupby("panel", dropna=False).groups.items():
        allowed = {
            str(x)
            for x in allowed_map.get(str(panel), [])
        }
        if not allowed:
            bad.loc[idxs] = True
            continue
    ...
    unexpected = df.loc[bad].copy()
    retained = df.loc[~bad].copy()

    if (
        not unexpected.empty
        and bool(cfg.get("strict_prep_root_map", True))
    ):
        roots = (
            unexpected[
                ["panel", "feature_source"]
            ]
            .drop_duplicates()
            .to_dict("records")
        )
        raise RuntimeError(
            "Unexpected panel/prep-root combinations detected: "
            f"{roots}. Inspect unexpected_prep_root_features.csv."
        )
...
def command_worker(cfg: Mapping, array_id: int) -> None:
    ...
    # ---------------------------------------------------------
    # Finalized pre-ranking hygiene
    # ---------------------------------------------------------
    n_initial = len(context_df)
    ...
    n_after_clinical = len(context_df)

    context_df, excluded_technical = remove_excluded_technical_features(
        context_df,
        cfg,
    )
    excluded_technical.to_csv(
        context_output / "excluded_technical_features.csv",
        index=False,
    )

    n_after_technical = len(context_df)

    # Write unexpected roots first; strict mode will fail instead of silently
    # accepting a new biological ontology.
    allowed_map = cfg.get(
        "expected_prep_roots_by_panel",
        EXPECTED_PREP_ROOTS_BY_PANEL,
    )
    unexpected_mask = pd.Series(False, index=context_df.index)
    for panel, idxs in context_df.groupby("panel", dropna=False).groups.items():
        allowed = {str(x) for x in allowed_map.get(str(panel), [])}
        unexpected_mask.loc[idxs] = ~context_df.loc[
            idxs, "feature_source"
        ].astype(str).isin(allowed)

    unexpected_preview = context_df.loc[unexpected_mask].copy()
    unexpected_preview.to_csv(
        context_output / "unexpected_prep_root_features.csv",
        index=False,
    )

    if not unexpected_preview.empty and bool(
        cfg.get("strict_prep_root_map", True)
    ):
        roots = (
            unexpected_preview[["panel", "feature_source"]]
            .drop_duplicates()
            .to_dict("records")
        )
        raise RuntimeError(
    ...
    context_df = context_df.loc[~unexpected_mask].copy()

    if context_df.empty:
        raise RuntimeError(
            "No candidate features remain after clinical/technical/root "
            f"hygiene filters: {context_input_path}"
        )
    ...
    log(
        "[PRE-RANK HYGIENE] "
        f"initial={n_initial} "
        f"clinical_removed={n_initial - n_after_clinical} "
        f"technical_removed={n_after_clinical - n_after_technical} "
    ...
    selected, transform_audit = choose_best_transform_for_context(
        context_df,
        cfg,
    )
    ...
    save_table(
        selected,
        context_output / "best_transform_features.parquet",
    )
    transform_audit.to_csv(
        context_output / "transform_selection_audit.csv",
        index=False,
    )

    summary = build_context_quality_summary(
        context_df,
        selected,
        array_id=array_id,
        cfg=cfg,
    ...
    summary.to_csv(
        context_output / "context_quality_summary.csv",
        index=False,
    )

    root_summary = build_root_candidate_summary(
        selected,
        array_id=array_id,
        cfg=cfg,
    )
    root_summary.to_csv(
        context_output / "root_candidate_summary.csv",
        index=False,
    )

    root_depth = build_root_candidate_depth_profile(
        selected,
        array_id=array_id,
        cfg=cfg,
    )
    root_depth.to_csv(
        context_output / "root_candidate_depth_profile.csv",
        index=False,
    )
    ...
    review_columns = [
        *CONTEXT_COLS,
        ...
        "root_rank_oof",
        "root_rank_stability",
        "root_rank_delta_clinical",
        "root_rank_completeness",
        "root_candidate_evidence_score",
        "root_evidence_score_weight_available",
        "review_evidence_tier",
    ...
    review_parts = [
        selected.sort_values("candidate_review_rank").head(top_n)
    ]
    root_top_n = int(cfg.get("top_n_for_manual_review_per_root", 100))
    for _, g in selected.groupby("feature_source", dropna=False):
        review_parts.append(
            g.sort_values("root_candidate_rank").head(root_top_n)
        )

    manual_review = (
        pd.concat(review_parts, ignore_index=True, sort=False)
        .drop_duplicates("feature_uid")
        .sort_values(
            ["feature_source", "root_candidate_rank", "candidate_review_rank"]
        )
    )

    manual_review[review_columns].to_csv(
        context_output / "top_features_for_manual_review.csv",
        index=False,
    )
...
def command_aggregate(cfg: Mapping) -> None:
    ...
    root_depth_parts: List[pd.DataFrame] = []
    ...
    for _, row in context_index.iterrows():
        ...
        root_summary_path = context_output / "root_candidate_summary.csv"
        root_depth_path = context_output / "root_candidate_depth_profile.csv"
        ...
        if root_summary_path.exists():
            root_summary_parts.append(pd.read_csv(root_summary_path))
        if root_depth_path.exists():
            root_depth_parts.append(pd.read_csv(root_depth_path))
    ...
    all_root_summary = pd.concat(root_summary_parts, ignore_index=True, sort=False) if root_summary_parts else pd.DataFrame()
    all_root_depth = pd.concat(root_depth_parts, ignore_index=True, sort=False) if root_depth_parts else pd.DataFrame()
    ...
    if not all_root_summary.empty:
        all_root_summary.sort_values(
            ["panel", "cohort", "endpoint", "feature_source"]
        ).to_csv(
            output_root / "all_context_root_candidate_summary.csv",
            index=False,
        )
    if not all_root_depth.empty:
        all_root_depth.sort_values(
            [
                "panel",
                "cohort",
                "endpoint",
            ...
            output_root / "all_context_root_candidate_depth_profile.csv",
            index=False,
        )
    ...
    if not all_selected.empty:
        root_transform_summary = (
            all_selected.groupby(
                CONTEXT_COLS + ["feature_source", "selected_transform_mode"],
                dropna=False,
            )
            .agg(n_variables=("feature_uid", "nunique"))
            .reset_index()
        )
        root_transform_summary.to_csv(
            output_root / "transform_selection_summary_by_context_and_root.csv",
            index=False,
        )
...
def default_config() -> dict:
    return {
        ...
        "output_root": "/projects/ovcare/users/nikolay_alabi/immuno/stage2_root_meta_modules_v1/stage2a_steps1_3_rootaware_v1",
        ...
        # Root-aware finalized design.
        "expected_prep_roots_by_panel": EXPECTED_PREP_ROOTS_BY_PANEL,
        "strict_prep_root_map": True,
        "excluded_technical_feature_patterns": [],
        "excluded_technical_feature_names": [],
        "root_review_depths": [5, 10, 15, 20, 30, 50, 75, 100],
        "top_n_for_manual_review_per_root": 100,
    }
